...
# ============================================================
# دالة استخراج المنحنيات (تدعم فصل المصفوفات الثنائية تلقائياً)
# ============================================================
                ...
                try:
                    ...
                    if isinstance(frame_data, np.ndarray) and frame_data.dtype.names:
                        for name in frame_data.dtype.names:
                            data = np.array(frame_data[name])
                            data = np.squeeze(data)
                            if data.size == 0: continue
                            
                            # فصل المصفوفات الثنائية (DIPLOG Pads) إلى منحنيات منفصلة
                            if data.ndim == 1:
                                curves[name] = data
                            elif data.ndim == 2:
        ...
        if not curves and hasattr(file_obj, 'curves'):
            ...
            if isinstance(curve_list, np.ndarray) and curve_list.dtype.names:
                for name in curve_list.dtype.names:
                    data = np.squeeze(np.array(curve_list[name]))
                    if data.size > 0:
                        if data.ndim == 1:
                            curves[name] = data
                        elif data.ndim == 2:
...
# ============================================================
# دالة التحويل (تتضمن معالجة أطوال المصفوفات المختلفة)
# ============================================================
def convert_file_fast(file_bytes, file_name, output_format="las", progress_callback=None):
    try:
        ...
        if output_format == "las":
            ...
            # --- معالجة الأبعاد الثنائية وتحويل النصوص لأرقام ---
            processed_curves = {}
            for name, data in curves.items():
                try:
                    # استبعاد البيانات التي لا يمكن تحويلها لأرقام
                    data = np.asarray(data).astype(float)
                    if data.ndim > 1:
                        data = data.flatten() # تسطيح الأبعاد الإضافية
                    processed_curves[name] = data
                except:
                    continue 

            if not processed_curves:
                return None, "لا توجد بيانات رقمية صالحة بعد المعالجة", None

            # --- إيجاد أقصى طول لمنع تعارض الأطوال (Concatenation Error) ---
            max_length = max(len(d) for d in processed_curves.values())
            
            # --- إضافة المنحنيات مع تعبئة الفراغات بـ NaN ---
            for name, data in processed_curves.items():
                if len(data) < max_length:
                    padded_data = np.full(max_length, np.nan)
                    padded_data[:len(data)] = data
                    las.append_curve(name, padded_data, unit="", descr=name)
                else:
                    las.append_curve(name, data[:max_length], unit="", descr=name)
            ...
            return output.getvalue().encode('utf-8'), None, processed_curves
