...
# دالة قراءة الملف وعرض البيانات
def read_log_file(file_bytes, file_name):
    """قراءة ملف الوج وعرض البيانات والرسوم البيانية"""
    try:
        ...
        with dlisio.dlis.load(tmp_path) as dlis_files:
            if not dlis_files:
                return None, "الملف فارغ أو غير صالح"
                ...
                for curve in frame.curves():
                    try:
                        ...
                        if data is not None and len(data) > 0:
                            curve_name = curve.name or f"Curve_{len(curves_in_frame)}"
            ...
            # معلومات البئر
            well_info = {
                'well_name': dlis_file.well_name or 'غير معروف',
...
def plot_comparison(original_data, converted_data, curve_names, max_curves=4):
    ...
    # إنشاء رسم بياني مع أعمدة متعددة
    fig = make_subplots(
        rows=1, 
        cols=n_curves * 2,  # كل منحنى له عمودين (أصلي + محول)
...
def plot_diff_comparison(original_data, converted_data, curve_names, max_curves=4):
    """عرض الرسم البياني للاختلافات بين البيانات الأصلية والمحولة"""
    common_curves = [c for c in curve_names if c in original_data and c in converted_data]
    common_curves = common_curves[:max_curves]
...
# دالة عرض إحصائيات المقارنة
def display_comparison_stats(comparison_results):
    """عرض إحصائيات المقارنة بشكل جميل"""
...
# دالة تحليل جودة الملف
def analyze_file_quality(file_bytes, file_name):
    """تحليل جودة ملف الوج وإرجاع تقرير مفصل"""
    report = {
        "file_name": file_name,
        "status": "✅ نجاح",
        "warnings": [],
    ...
    try:
        ...
        with dlisio.dlis.load(tmp_path) as dlis_files:
            if not dlis_files:
                report["status"] = "❌ فشل"
                report["errors"].append("الملف فارغ أو غير صالح")
            ...
            # معلومات البئر
            report["info"]["well_name"] = dlis_file.well_name or "غير معروف"
            ...
            # حجم الملف
            if report["file_size_kb"] > 10000:  # أكبر من 10 ميجا
    ...
    try:
        ...
        with dlisio.dlis.load(tmp_path) as dlis_files:
            ...
            if output_format == "las":
                ...
                if frame is None:
                    return None, "لا توجد منحنيات"
                
                for curve in frame.curves():
                    try:
                        data = curve.curves()
                        if data is not None and len(data) > 0:
                            curves_data[curve.name] = data
                    except:
                        continue
                
                if not curves_data:
                    return None, "لا توجد بيانات قابلة للقراءة"
                
                # إنشاء LAS بسرعة
                las = lasio.LASFile()
                las.well = lasio.WellItem("WELL", value=dlis_file.well_name or "UNKNOWN")
                ...
                if len(las.keys()) > 0:
                    depth = np.arange(len(las.data[las.keys()[0]])) * 0.1524
...
uploaded_files = st.file_uploader(
    ...
    help="اختر ملف واحد أو أكثر للتحويل"
)
...
if uploaded_files:
    ...
    if st.button("🚀 تحويل الكل", type="primary", use_container_width=True):
        ...
        for report in quality_reports:
            quality_class = "quality-good" if report["quality_score"] >= 80 else "quality-warning" if report["quality_score"] >= 50 else "quality-error"
        ...
        for i, file in enumerate(uploaded_files):
            ...
            # قراءة البيانات الأصلية للمقارنة
            original_data = None
