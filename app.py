...
# ============================================================
# دالة التحويل إلى LAS (تم إصلاح مشكلة WellItem)
# ============================================================
def convert_file_fast(file_bytes, file_name, output_format="las", progress_callback=None):
    try:
        ...
        if output_format == "las":
            # إنشاء ملف LAS باستخدام الطريقة الجديدة
            las = lasio.LASFile()
            
            # إضافة معلومات البئر بالطريقة الصحيحة (بدون WellItem)
            well_name = getattr(dlis_file, 'well_name', 'UNKNOWN')
            
            # الطريقة الجديدة: التعامل مع las.well كقاموس
            if hasattr(las, 'well'):
                # في الإصدارات الجديدة، las.well هو كائن يمكن التعامل معه كـ dict
                if hasattr(las.well, 'WELL'):
                    las.well.WELL.value = well_name
                elif hasattr(las.well, 'items'):
                    # بعض الإصدارات تسمح بالتعامل كـ dict
            ...
            for name, data in curves.items():
                if len(data) > 0:
                    try:
                        # محاولة إضافة المنحنى بالطريقة العادية
                        las.append_curve(name, data, unit="", descr=name)
                    except Exception as e:
                        # لو فشلت، نجرب طريقة بديلة
            ...
            if len(las.keys()) > 0:
                ...
                if len(las.data[first_key]) > 0:
                    ...
                    # التحقق إذا كان DEPT موجوداً بالفعل
